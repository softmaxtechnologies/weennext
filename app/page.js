...
import {
  ...
  setDoc, deleteDoc, query, where, orderBy, Timestamp, writeBatch
...
const ensureUserInFirestore = async (user) => {
  ...
  if (!snap.exists()) {
    await setDoc(ref, {
      ...
      photoUrl: user.photoURL || '', isAdmin: false, userType: 'customer',
...
const Icon = {
  ...
  Star: ({ filled }) => <svg className={`w-4 h-4 ${filled ? 'text-amber-400 fill-amber-400' : 'text-gray-300 fill-gray-300'}`} viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>,
  ...
  Copy: () => <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>,
};
...
const Stars = ({ rating, size = 4 }) => (
  <div className="flex gap-0.5">
    {[1,2,3,4,5].map(i => (
      <svg key={i} className={`w-${size} h-${size} ${i <= Math.round(rating) ? 'text-amber-400 fill-amber-400' : 'text-gray-200 fill-gray-200'}`} viewBox="0 0 24 24">
        ...
        <div className="p-4 border-t">
          {address ? (
            <div className="flex items-start gap-3 bg-violet-50 rounded-xl p-3 mb-3">
              <div className="text-violet-600 mt-0.5"><Icon.Location /></div>
              <div>
    ...
    <div className="min-h-screen bg-gray-50 animate-fadeIn">
      ...
      <div className="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
          <button onClick={onBack} className="flex items-center gap-2 text-sm text-gray-600 hover:text-violet-600 transition font-medium">
            <Icon.Back /> Back
          </button>
          ...
          <div className="ml-auto">
            <button
              onClick={handleShare}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition ${copied ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-600 hover:bg-violet-50 hover:text-violet-600'}`}
            >
        ...
        <div className="grid md:grid-cols-2 gap-8 bg-white rounded-3xl shadow-sm overflow-hidden">
          ...
          <div className="p-6">
            <div className="bg-gray-50 rounded-2xl overflow-hidden aspect-square flex items-center justify-center mb-4">
              {product.imageUrl ? (
                ...
                <div className="text-gray-300"><Icon.Package /></div>
              )}
            </div>
            <span className="inline-flex px-3 py-1 bg-violet-100 text-violet-700 text-xs font-semibold rounded-full">{product.category}</span>
          </div>
          ...
          <div className="p-6 flex flex-col">
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <span className="text-sm text-gray-500">{avgRating} ({productFeedbacks.length || 0} reviews)</span>
            </div>
            <div className="mt-4">
              <span className="text-4xl font-extrabold text-violet-700">₹{product.price.toLocaleString()}</span>
            </div>
            <p className="text-gray-500 text-sm mt-3 leading-relaxed">{product.description || 'Premium quality product for everyday use.'}</p>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3">
              {[['🚚', 'Free Delivery', 'Orders above ₹499'], ['↩️', 'Easy Returns', '7 day policy'], ['🔒', 'Secure Pay', 'UPI & COD']].map(([icon, title, sub]) => (
    ...
    >
      <div className="relative bg-gray-50 aspect-square overflow-hidden">
        {product.imageUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-gray-300"><Icon.Package /></div>
        )}
        {product.stock <= 5 && product.stock > 0 && (
          <span className="absolute top-2 left-2 bg-amber-500 text-white text-xs px-2 py-0.5 rounded-full font-semibold">Only {product.stock} left</span>
        )}
        {product.stock === 0 && (
      ...
      <div className="p-3">
        <span className="text-xs text-violet-600 font-semibold bg-violet-50 px-2 py-0.5 rounded-full">{product.category}</span>
            ...
            <div key={item.productId} className="flex gap-3 bg-gray-50 rounded-2xl p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <button onClick={() => onUpdateQty(item.productId, item.quantity - 1)} className="w-6 h-6 bg-white rounded-lg shadow-sm flex items-center justify-center text-gray-600 font-bold hover:bg-violet-50">−</button>
...
const CheckoutModal = ({ cart, user, onClose, onOrderPlaced }) => {
  ...
  const handlePlaceOrder = async (txnId = null) => {
    ...
    try {
      ...
      const orderData = {
        userId: user.uid, userEmail: user.email, userName: user.displayName, orderId,
        items, totalAmount: total,
        status: paymentMethod === 'qr' && txnId ? 'payment_verification' : 'pending',
        paymentMethod, paymentStatus: paymentMethod === 'cod' ? 'pending' : 'verification_pending',
  ...
  return (
    <>
      {showMap && <MapPickerModal onSelect={loc => { setMapLocation(loc); setDeliveryAddress(a => ({ ...a, street: loc.address })); setShowMap(false); }} onClose={() => setShowMap(false)} />}
      {showQR && <QRPaymentModal amount={total} onSuccess={async (txnId) => { setShowQR(false); await handlePlaceOrder(txnId); }} onClose={() => setShowQR(false)} />}
        ...
        <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b">
            ...
            <div className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${step >= 1 ? 'bg-violet-600 text-white' : 'bg-gray-100 text-gray-400'}`}>1</div>
              <div className={`w-6 h-0.5 ${step >= 2 ? 'bg-violet-400' : 'bg-gray-200'}`} />
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${step >= 2 ? 'bg-violet-600 text-white' : 'bg-gray-100 text-gray-400'}`}>2</div>
            </div>
          ...
          <div className="p-6">
            {step === 1 && (
              <div className="space-y-3">
                ...
                {mapLocation && <p className="text-xs text-green-600 bg-green-50 rounded-lg px-3 py-2 flex items-start gap-1.5"><Icon.Check /><span className="line-clamp-2">{mapLocation.address}</span></p>}
                <input type="text" placeholder="Street / House No *" value={deliveryAddress.street} onChange={e => setDeliveryAddress(a => ({ ...a, street: e.target.value }))} className="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-violet-500 outline-none" />
                <div className="grid grid-cols-2 gap-2">
                  <input type="text" placeholder="City *" value={deliveryAddress.city} onChange={e => setDeliveryAddress(a => ({ ...a, city: e.target.value }))} className="border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-violet-500 outline-none" />
                  <input type="text" placeholder="State" value={deliveryAddress.state} onChange={e => setDeliveryAddress(a => ({ ...a, state: e.target.value }))} className="border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-violet-500 outline-none" />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <input type="text" placeholder="Pincode *" value={deliveryAddress.pincode} onChange={e => setDeliveryAddress(a => ({ ...a, pincode: e.target.value }))} className="border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-violet-500 outline-none" />
                  <input type="text" placeholder="Phone" value={deliveryAddress.phone} onChange={e => setDeliveryAddress(a => ({ ...a, phone: e.target.value }))} className="border-2 border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:border-violet-500 outline-none" />
                </div>
                <button
                  onClick={() => { if (!deliveryAddress.street || !deliveryAddress.city || !deliveryAddress.pincode) { alert('Fill required fields'); return; } setStep(2); }}
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {[
                    { id: 'cod', label: 'Cash on Delivery', sub: 'Pay when your order arrives', Icon: Icon.COD, color: 'green' },
                    { id: 'qr', label: 'UPI / QR Code', sub: 'Pay now via any UPI app', Icon: Icon.QR, color: 'violet' }
                  ].map(opt => (
                    <button
                      key={opt.id}
                      onClick={() => setPaymentMethod(opt.id)}
                      className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 transition ${paymentMethod === opt.id ? `border-${opt.color}-500 bg-${opt.color}-50` : 'border-gray-200 hover:border-gray-300'}`}
                    >
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${paymentMethod === opt.id ? `bg-${opt.color}-100 text-${opt.color}-600` : 'bg-gray-100 text-gray-400'}`}>
                        <opt.Icon />
                      </div>
                      ...
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${paymentMethod === opt.id ? `border-${opt.color}-500 bg-${opt.color}-500` : 'border-gray-300'}`}>
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (window.google?.accounts?.id) loadOneTap();
      ...
      s.async = true; s.onload = loadOneTap;
  ...
  const handleGoogleSignIn = async () => {
    ...
    } catch (e) { console.error(e); alert('Sign in failed. Try again.'); }
  };
  ...
  const handleAddToCart = (product, qty = 1) => {
    ...
    else c.push({ productId: product.id, name: product.name, price: product.price, imageUrl: product.imageUrl, quantity: qty, stock: product.stock });
    saveCart(c);
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      await addDoc(collection(db, 'feedbacks'), {
        ...
        productId: null, createdAt: Timestamp.now()
      });
  ...
  if (selectedProduct) return (
    <ProductDetailPage
      product={selectedProduct} user={user} feedbacks={feedbacks}
      onBack={() => { setSelectedProduct(null); window.history.pushState({}, '', window.location.pathname); }}
      onAddToCart={(p, qty) => { handleAddToCart(p, qty); showToast('Added to cart!'); }}
  ...
  return (
    <>
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-[300] animate-toastIn px-5 py-3 rounded-full shadow-xl text-white text-sm font-semibold flex items-center gap-2 ${toast.type === 'info' ? 'bg-gray-700' : 'bg-gradient-to-r from-violet-600 to-indigo-600'}`}>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {user ? (
                <div className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-1.5">
                  ...
                  <button onClick={handleLogout} className="text-xs text-red-500 hover:text-red-600 font-medium hidden sm:block">Sign out</button>
                </div>
              ) : (
                <button onClick={() => setShowLoginPopup(true)} className="flex items-center gap-2 bg-violet-600 text-white text-sm font-semibold px-4 py-2 rounded-xl hover:bg-violet-700 transition">
                  <span>Sign In</span>
                </button>
              )}
              <button
          ...
          <div className="flex gap-1 pb-2 overflow-x-auto category-scroll">
            {categories.map(cat => (
              <button
                ...
                className={`flex-shrink-0 px-4 py-1.5 rounded-full text-sm font-medium transition ${selectedCategory === cat ? 'bg-violet-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'}`}
              >
        ...
        <div className="flex gap-0 border-b border-gray-100 mt-4 mb-6">
          {[['shop', '🛍️ Shop'], ['orders', '📦 My Orders'], ['reviews', '💬 Reviews']].map(([tab, label]) => (
            <button
              ...
              className={`px-5 py-3 text-sm font-semibold border-b-2 transition ${activeTab === tab ? 'border-violet-600 text-violet-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
            >
              {label}
            </button>
          ))}
          ...
          <div className="animate-fadeIn">
            ...
            {!searchTerm && selectedCategory === 'all' && (
              <div className="relative rounded-3xl overflow-hidden mb-8 h-52 md:h-72">
                {carouselSlides.map((slide, idx) => (
                  <div
                    key={idx}
                    className={`absolute inset-0 bg-gradient-to-br ${slide.bg} flex items-center transition-opacity duration-700 ${currentSlide === idx ? 'opacity-100' : 'opacity-0'}`}
                  >
                    ...
                    <div className="absolute right-8 bottom-8 hidden md:flex gap-2 opacity-20">
                      {[...Array(4)].map((_, i) => (
                        <div key={i} className="w-16 h-16 rounded-full bg-white" style={{ transform: `scale(${1 - i * 0.2})` }} />
                      ))}
                    </div>
                  </div>
                ))}
            ...
            <div className="flex items-center justify-between mb-5">
              ...
              <div className="flex items-center gap-2 bg-gray-100 rounded-xl p-1">
                <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg transition ${viewMode === 'grid' ? 'bg-white shadow-sm text-violet-600' : 'text-gray-400'}`}><Icon.Grid /></button>
                <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition ${viewMode === 'list' ? 'bg-white shadow-sm text-violet-600' : 'text-gray-400'}`}><Icon.List /></button>
              </div>
            </div>
            ...
            {filtered.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-3xl shadow-sm">
                ...
                <button onClick={() => { setSearchTerm(''); setSelectedCategory('all'); }} className="mt-4 text-violet-600 text-sm font-medium">Clear filters</button>
              </div>
            ) : viewMode === 'grid' ? (
          ...
          <div className="animate-fadeIn">
            {!user ? (
              <div className="text-center py-20 bg-white rounded-3xl shadow-sm">
                ...
                <button onClick={handleGoogleSignIn} className="mt-6 bg-violet-600 text-white px-8 py-3 rounded-xl font-semibold">Sign In with Google</button>
              </div>
            ) : orders.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-3xl shadow-sm">
                <p className="text-6xl mb-4">📦</p>
                ...
                <button onClick={() => setActiveTab('shop')} className="mt-6 bg-violet-600 text-white px-8 py-3 rounded-xl font-semibold">Shop Now</button>
              </div>
            ) : (
              <div className="space-y-4">
                {orders.map(order => (
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1.5">
                          ...
                          {order.paymentMethod === 'qr' && (
                            <span className="text-xs px-2 py-0.5 rounded-full bg-violet-100 text-violet-600 font-medium">UPI</span>
                          )}
                          {order.paymentMethod === 'cod' && (
                            <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 font-medium">COD</span>
                          )}
                        </div>
                      </div>
              ...
              {user ? (
                <div>
                  <div className="flex items-center gap-1 mb-3">
                    {[1,2,3,4,5].map(r => (
                      <button key={r} onClick={() => setNewFeedback(f => ({ ...f, rating: r }))} className="text-2xl transition">
                        <span className={r <= newFeedback.rating ? 'text-amber-400' : 'text-gray-200'}>★</span>
                      </button>
                    ))}
                  ...
                  <textarea
                    ...
                    rows="3"
                  />
                  <button onClick={handleSubmitFeedback} className="mt-2 bg-violet-600 text-white px-5 py-2 rounded-xl text-sm font-semibold">Post Review</button>
                </div>
              ) : (
                <button onClick={() => setShowLoginPopup(true)} className="text-violet-600 font-semibold text-sm hover:underline">Sign in to leave a review →</button>
              )}
            </div>
            {feedbacks.length === 0 ? (
